...
import BasePlugin from '../node_modules/handsontable/src/plugins/_base';
import { registerPlugin } from '../node_modules/handsontable/src/plugins';
 ...
 */
class dataSourceConnectorPlugin extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('beforeColumnSort', () => {
      return false;
    });
    this.addHook('beforeFilter', () => {
      return false;
    });
  ...
  onAfterColumnSort(column, order) {
    ...
    this._get(controllerUrl + "/aftercolumnsort" + uri.string()).then(response => {
  ...
  onAfterInit() {
    ...
    this._get(this.controllerUrl + '/settings').then(response => {
      this.hot.updateSettings(response.data);
    });
    this._get(this.controllerUrl + '/data').then(response => {
      this._loadData(response);
    });
  }
  ...
  _post(url, data) {
    ...
    return this._request(request).then(value => {
...
export { dataSourceConnectorPlugin };
